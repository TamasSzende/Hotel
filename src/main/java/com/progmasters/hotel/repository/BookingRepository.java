...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b,r.startDate FROM Booking b JOIN b.roomReservations r WHERE r.room.id = :room_id " +
            ...
            "ORDER BY r.startDate")
    List<Booking> findFutureByRoomId(@Param("room_id") Long roomId, @Param("current_date") LocalDate date);

    @Query("SELECT b,r.startDate FROM Booking b JOIN b.roomReservations r WHERE r.room.id = :room_id " +
            "AND r.endDate < :current_date " +
            "ORDER BY r.startDate DESC")
    List<Booking> findPastByRoomId(@Param("room_id") Long roomId, @Param("current_date") LocalDate date);
    ...
    @Query("SELECT DISTINCT b,reservations.startDate FROM Booking b JOIN b.roomReservations reservations JOIN reservations.room room JOIN room.hotel hotel WHERE hotel.id = :hotel_id " +
            "AND reservations.startDate <= :current_date AND reservations.endDate >= :current_date " +
            "ORDER BY reservations.startDate")
    List<Booking> findCurrentByHotelId(@Param("hotel_id") Long hotelId, @Param("current_date") LocalDate date);

    @Query("SELECT DISTINCT b,reservations.startDate FROM Booking b JOIN b.roomReservations reservations JOIN reservations.room room JOIN room.hotel hotel WHERE hotel.id = :hotel_id " +
            "AND reservations.startDate > :current_date " +
            "ORDER BY reservations.startDate")
    List<Booking> findFutureByHotelId(@Param("hotel_id") Long hotelId, @Param("current_date") LocalDate date);

    @Query("SELECT DISTINCT b,reservations.startDate FROM Booking b JOIN b.roomReservations reservations JOIN reservations.room room JOIN room.hotel hotel WHERE hotel.id = :hotel_id " +
            "AND reservations.endDate < :current_date " +
            "ORDER BY reservations.startDate DESC")
    List<Booking> findPastByHotelId(@Param("hotel_id") Long hotelId, @Param("current_date") LocalDate date);
